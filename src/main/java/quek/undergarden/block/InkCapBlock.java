...
public class InkCapBlock extends Block {
	...
	public void animateTick(BlockState state, Level level, BlockPos pos, RandomSource random) {
		if (level.getRandom().nextInt(35) == 0 && !level.getBlockState(pos.below()).is(UGBlocks.SEEPING_INK.get())) {
	...
	private void spawnFluidParticle(Level level, double x1, double x2, double z1, double z2, double y) {
		level.addParticle(UGParticleTypes.DRIPPING_INK.get(), Mth.lerp(level.getRandom().nextDouble(), x1, x2), y, Mth.lerp(level.getRandom().nextDouble(), z1, z2), 0.0D, 0.0D, 0.0D);
	}
}
